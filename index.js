...
function createMenuItem(strName, intPrice, strCat){
    const item = {name: strName, price: intPrice, category: strCat};
    return item;
}
...
*/
console.log(createMenuItem("boogers", 0, "snack"));
console.log(createMenuItem("burgers", 12, "lunch"));
...
const burger = {
  ...
  price: 18, 
  category: "Lunch", 
  discount: function(dType){
    if ((dType === `teacher`) || (dType === `student`)){
...
*/
 ...
 function getReviewByRating(arr, score) {
    ...
    for (let i=0; i<arr.length; i++){
      if (Math.floor(arr[i].rating) === score)
        subset.push(arr[i]);
    }
    return subset;
  }
...
function getLongReviews(arr) {
  let subset = [];
  for (let i=0; i<arr.length; i++){
    if (wordCount(arr[i].feedback) > 15){
      subset.push(arr[i]);
    }
  }
  return subset;
  ...
  function wordCount(str){
    ...
    str = str.trim();
    // replace double spacing with single spaces
    str = str.replace(/ {1,}/g," ");
